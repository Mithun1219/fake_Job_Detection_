...
SUSPICIOUS_PATTERNS = [
    "registration fee", "no interview", "earn money", "work from home",
    "whatsapp", "urgent hiring", "investment required", "limited slots",
    "instant joining", "guaranteed income", "telegram", "crypto",
    "no experience required", "easy money", "wire transfer", "cash advance",
    "package forwarding", "payment processing"
]
...
def predict(data: PredictRequest):
    try:
        ...
        # Final decision - Aggressive Override for Scams
        if len(highlight_words) >= 1:
            fake_prob = max(fake_prob, 85)
            # Suppress false authenticity when scam patterns exist
            real_prob = min(real_prob, 15)
            label = "FAKE"
            risk = "high"
        elif fake_prob >= 50:
            label = "FAKE"
            risk = "medium" if fake_prob < 80 else "high"
        else:
            label = "REAL"
            risk = "low"

        # Re-calc confidence exactly so UI looks consistent
        total = fake_prob + real_prob
        fake_prob = round((fake_prob / total) * 100, 2)
        real_prob = round((real_prob / total) * 100, 2)
